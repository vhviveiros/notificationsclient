...
/**
 * Destructure the Wake-on-LAN (WoL) module from NativeModules.
 */
...
export class ClientWebSocket {
     ...
     * @returns {Promise<string>} Resolves with a success message if successful.
     */
    async sendWoL(macAddress: string, targetIp: string, targetPort: number): Promise<string> {
        try {
